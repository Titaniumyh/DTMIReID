...
class PatchEmbed_overlap(nn.Module):
    ...
    def forward(self, x):
        ...
        x1 = x.flatten(2).transpose(1, 2) # [64, 8, 768]
        return x1, x
...
class TransReID(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.norms = nn.ModuleList([nn.LayerNorm(embed_dim) for i in range(depth)])
    ...
    def forward_features(self, x, camera_id, view_id):
        ...
        if self.local_feature:
            ...
            for blk in self.blocks:
                ...
                # if i % 3 == 0:
                output.append(x)
            output = [self.norms[i](output[i]) for i in range(12)]
            return output, f
            # x = self.norm(x)
    ...
    def forward(self, x, cam_label=None, view_label=None):
        x, f = self.forward_features(x, cam_label, view_label)
        return x, self.pos_embed, f
